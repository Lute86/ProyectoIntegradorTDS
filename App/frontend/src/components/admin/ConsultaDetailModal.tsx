    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-gray-900">Detalle de Consulta</h2>
        ...
        <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-xl">
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Nombre</p>
            <p className="text-sm font-medium text-gray-800">{consulta.nombre}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Email</p>
            <p className="text-sm font-medium text-gray-800">{consulta.email}</p>
          </div>
          <div className="col-span-2">
            <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Asunto</p>
            <p className="text-sm font-medium text-gray-800">{consulta.asunto}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Fecha</p>
            <p className="text-sm text-gray-600">{new Date(consulta.createdAt).toLocaleString('es-AR')}</p>
          </div>
          <div>
        ...
        <div>
          <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-2">Mensaje</p>
          <div className="p-4 bg-white border border-gray-200 rounded-xl text-sm text-gray-700 whitespace-pre-wrap break-words overflow-y-auto max-h-60">{consulta.mensaje}</div>
        </div>
        ...
        <div>
          ...
          {consulta.respondido ? (
            <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 whitespace-pre-wrap break-words overflow-y-auto max-h-40">{consulta.respuesta || 'Sin respuesta registrada.'}</div>
          ) : (
        ...
        <div className="flex gap-3 pt-2">
          <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 text-sm font-semibold rounded-lg hover:bg-gray-50 transition">Cerrar</button>
          {!consulta.respondido && (
            <>
