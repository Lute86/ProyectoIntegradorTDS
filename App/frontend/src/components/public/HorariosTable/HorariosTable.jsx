export default function HorariosTable({ horarios }) {
  if (!horarios || horarios.length === 0) {
    return     <p className="text-slate-500 dark:text-white/50 text-sm">No hay horarios disponibles.</p>
  }
        ...
        <tbody>
          {horarios.map((h, i) => (
            <tr key={i} className="border-b border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/5">
              ...
              <td className="p-3 text-sm text-slate-600 dark:text-white/70">{h.horario}</td>
              <td className="p-3 text-sm text-slate-600 dark:text-white/70">{h.aula}</td>
            </tr>
          ))}
        </tbody>
      </table>
