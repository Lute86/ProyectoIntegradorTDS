...
const BuscarMateriaModal = ({ isOpen, onClose, carreraId, onAsignada, maxCuatri = 12, cuatrimestrePrefijado, materiasAsignadas = new Set() }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={handleClose}>
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-200 dark:border-slate-700 w-full max-w-lg mx-4 max-h-[80vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-5 border-b border-gray-200 dark:border-slate-700">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-bold text-gray-900 dark:text-slate-100">Agregar materias al plan</h2>
            <button onClick={handleClose} className="text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:hover:text-slate-300 text-xl leading-none">&times;</button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-1">
          {loading ? (
            <div className="space-y-2 animate-pulse">
              {[1, 2, 3, 4].map((i) => <div key={i} className="h-8 bg-gray-100 dark:bg-slate-700/50 rounded" />)}
            </div>
          ) : filtradas.length === 0 ? (
            ...
            filtradas.map((m) => {
              ...
              if (yaAsignada) {
                return (
                  <div key={m.id}
                    className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-400 dark:text-slate-500 bg-gray-50 dark:bg-slate-700/50"
                  >
                    <span className="text-green-400 text-xs">✓</span>
                    {m.nombre}
                    <span className="text-[10px] text-gray-300 dark:text-slate-500 ml-auto">ya asignada</span>
                  </div>
                )
              }
              ...
              return (
                <label key={m.id}
                  className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm cursor-pointer transition-colors ${
                    checked ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-700'
                  }`}
                >
        ...
        {seleccionadas.size > 0 && (
          <div className="border-t border-gray-200 dark:border-slate-700 p-5 space-y-3">
            <p className="text-sm font-semibold text-gray-800 dark:text-slate-100">
              {seleccionadas.size} materia{seleccionadas.size !== 1 ? 's' : ''} seleccionada{seleccionadas.size !== 1 ? 's' : ''}
            </p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-slate-300 mb-1">Cuatrimestre</label>
                {cuatrimestrePrefijado ? (
                  <p className="text-sm font-semibold text-gray-800 dark:text-slate-100 py-1.5">Cuatrimestre {cuatrimestrePrefijado}</p>
                ) : (
                  <>
                    <input type="number" min={1} max={maxCuatri} value={cuatrimestre} onChange={(e) => {
                      ...
                      className="w-full px-3 py-1.5 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-slate-300 mb-1">Carga horaria semanal</label>
                <input type="number" min={1} value={cargaHoraria} onChange={(e) => setCargaHoraria(e.target.value)}
                  placeholder="hs"
                  className="w-full px-3 py-1.5 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 dark:placeholder:text-slate-400 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  required
                />
              </div>
            </div>
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={handleClose}
                className="flex-1 px-3 py-2 text-sm font-semibold text-gray-600 dark:text-slate-300 bg-gray-100 dark:bg-slate-700/50 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-lg"
              >Cancelar</button>
