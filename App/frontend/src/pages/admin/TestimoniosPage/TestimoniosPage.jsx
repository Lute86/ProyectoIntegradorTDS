...
const TestimoniosPage = () => {
  const { testimonios, isLoading, error, fetchTestimonios, deleteTestimonio } = useTestimoniosStore();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [testimonioToEdit, setTestimonioToEdit] = useState(null);
  ...
  const abrirModalEditar = (testimonio) => {
  ...
  const handleEliminar = async (id) => {
    if (window.confirm('Confirma que desea eliminar este testimonio?')) {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
        ...
        <DataTable searchable columns={columns} data={testimonios} emptyMessage="No hay testimonios registrados." />
        }
      </div>

      <TestimonioFormModal isOpen={isModalOpen} onClose={cerrarModal} testimonioToEdit={testimonioToEdit} />
    </div>);
