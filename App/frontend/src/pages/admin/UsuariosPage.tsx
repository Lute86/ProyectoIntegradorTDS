...
const UsuariosPage = () => {
  ...
  const abrirModalEditar = (user: User) => { setUsuarioToEdit(user); setIsModalOpen(true); };
      ...
      accessor: (user) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="font-semibold text-gray-800 text-sm">{user.nombre} {user.apellido}</span>
    ...
    },
    {
      ...
      accessor: (user) => (
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-medium ${user.activo ? 'text-green-700' : 'text-gray-500'}`}>{user.activo ? 'Activo' : 'Inactivo'}</span>
        </div>
      ),
    },
    {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Gestion de Usuarios</h1>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <input type="text" value={confirmText} onChange={(e) => setConfirmText(e.target.value)}
              placeholder="Escriba ELIMINAR" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition" />
            <div className="flex gap-3 pt-2">
              <button onClick={() => { setDeleteTarget(null); setConfirmText(''); }}
                className="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 text-sm font-semibold rounded-lg hover:bg-gray-50 transition">Cancelar</button>
