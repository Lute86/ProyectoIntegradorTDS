      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <nav className="flex items-center gap-2 text-sm text-slate-500">
            ...
            <span className="text-slate-800 font-medium">Noticias</span>
          </nav>
        </div>
      </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm p-5">
              ...
              <div className="relative">
                <input type="text" placeholder="Buscar noticias..." value={search} onChange={handleSearch}
            ...
            <div className="bg-white rounded-xl shadow-sm p-5">
              <h3 className="font-bold text-slate-900 mb-4 pb-2 border-b border-slate-200">Categorías</h3>
              <ul className="space-y-1">
                {categorias.map((cat) => (
                  <li key={cat.nombre}>
                    <button onClick={() => handleCategoryFilter(cat.nombre)}
                      className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${selectedCategory === cat.nombre ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-slate-600 hover:bg-slate-50'}`}>
                      <span>{cat.nombre}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full ${selectedCategory === cat.nombre ? 'bg-blue-200 text-blue-800' : 'bg-slate-100 text-slate-500'}`}>{cat.count}</span>
                    </button>
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-white rounded-xl shadow-sm p-5">
              <h3 className="font-bold text-slate-900 mb-4 pb-2 border-b border-slate-200">Noticias Destacadas</h3>
              <ul className="space-y-3">
                {MOCK_NOTICIAS.slice(0, 3).map((n) => (
                  <li key={n.id}>
                    <Link to={`/noticias/${n.slug}`} className="block text-sm font-medium text-slate-700 hover:text-blue-600 leading-snug">{n.titulo}</Link>
                  </li>
                ))}
              </ul>
            </div>
